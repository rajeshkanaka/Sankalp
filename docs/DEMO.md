...
- **Isolation:** the selected Docker context and any exported `DOCKER_HOST` must both use a local Unix socket. Unset a remote `DOCKER_HOST` before retrying; an empty/unknown context endpoint also fails closed. Inherited shell database, origin, port, clock and `SUPABASE_SERVICES_HOSTNAME` settings are removed from child processes so the generated checkout configuration is authoritative. Existing manually edited `.env.local` values retain the local wrapper's ownership protection.
