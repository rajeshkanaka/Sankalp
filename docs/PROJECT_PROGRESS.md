...
| Exact next action | Resume SK-008 on the named feature/workers: core fixes and storage tests are integrated; public shell and app mounting build. Integrate next UI lifecycle/hydration-test fixes and deterministic network-failure helper, rebuild, then rerun focused offline/fallback/revision/smoke workflows. Read SK-008 midday checkpoint for actual mixed test outcomes; do not repeat completed recovery work. |
