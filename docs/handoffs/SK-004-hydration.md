...
- This is explicitly an online hydration/recovery scenario with `serviceWorkers: 'block'`; separate offline workflows retain service-worker coverage. The script gate is released in `finally`, then `page.unrouteAll({ behavior: 'wait' })` drains the already-running handlers. At that point the script gate is this scenario's only page route; metadata conflict routing is installed afterward. No arbitrary sleep, test retry, weaker expectation or alternate application implementation was added.

## Integration feedback and test correction

The coordinator's first actual root run, recorded in ignored `artifacts/offline-fallback-ui.log`, reported all three fallback cases plus HTTP passing but all three revision cases failing. Chromium/Firefox exposed a test cleanup race: releasing the promise and immediately calling `page.unroute` could retire a route before its async `route.continue` finished. WebKit's active service worker bypassed the page's script routing, leaving the held-script count at zero. These are failures of the initial regression harness and remain failed evidence, not application success.

The correction uses Playwright's documented [waiting route cleanup](https://playwright.dev/docs/api/class-page#page-unroute-all) and [service-worker blocking for interception](https://playwright.dev/docs/service-workers#how-to-disable-service-workers), verified 2026-09-06 against the pinned 1.63 API. All pre-hydration, payload, conflict and schedule assertions remain intact. The corrected actual-app run is pending coordinator execution. Hosted run [34017563905](https://github.com/rajeshkanaka/Sankalp/actions/runs/34017563905) on the earlier PR head `340d126` also concluded failure; its detailed cause was not inspected in this follow-up.
