...
Each save retains an internally frozen operation/base/payload envelope and gives the wrapper a clone. An unchanged retry preserves exactly that request; any user edit or explicit conflict resolution clears it for a fresh operation. Duplicate submissions are synchronously guarded; pending controls are disabled. Failures keep raw input. A 409 authorized current view or newer same-journey props offers **Keep my choices** (retain raw values, explicitly adopt the latest base, require a subsequent save only when those values differ) and **Use latest saved settings** (explicitly replace local values). Foreign/malformed current views are not rendered. Successful responses are validated before a saved claim. Parent wrappers should throw the existing `RequestError` with `status: 409` and the authorized full `current` view for conflicts.

No-change follow-up, based on worker commit `41a0d21`: valid choices equal to the currently accepted saved preferences disable Save and are also a no-op at the submit boundary, with “These choices match the saved settings.” No operation ID or request is created for them. **Keep my choices** uses the same rule when the reviewed current preferences already match. Equality covers all persisted fields and preserves offset array order, matching the changed-JSON revision contract; raw numeric spelling may differ without a persisted change. Invalid raw inputs remain editable and eligible for validation. A retained failed-attempt envelope bypasses no-change suppression, so an unchanged uncertain retry keeps its original identity/payload/base. Neither path fabricates a revision increment or a save result.
...
- `node --import tsx --test src/features/reminders/preferences/model.test.ts` — **PASS, 9 focused tests** after the no-change follow-up: initial/normalized equivalent choices; changed settings and reviewed equal choices; invalid-input and retained-retry exceptions; every persisted equality field; plus the original six validation, immutable-envelope, response and timezone tests.
...
The nine model tests use Node's built-in runner and are outside the existing `tests/unit` Vitest glob because this worker may edit only its feature directory. Coordinator must include the exact Node command in the integration/CI gate or relocate those tests within coordinator ownership. No new test dependency is needed. Scoped lint, full TypeScript and formatting checks were rerun successfully after the no-change follow-up.
...
2. Run the focused Node command above, normal static/build/database gates and actual `@M4-reminders` Playwright workflows once routes exist. Assert disabled defaults/no automatic permission, initial unchanged Save disabled with zero PUTs, custom offsets 30/5/0, duplicate/empty/range errors, invalid quiet text retained, midnight quiet-hour suppression, full timezone preview, saved revision/reload, real server conflict with both explicit choices, and unchanged-request retry after clearly labeled failed/lost transport. After **Keep my choices** matches the latest settings exactly, assert Save stays disabled with no extra PUT or revision increment. Compare operation ID, base revision and payload exactly on actual retries; newer edits must use a different ID.
